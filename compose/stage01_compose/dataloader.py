...
class SkylineFullSongTransformerDataset(Dataset):
  def __init__(self, data_dir, vocab_file, 
    ...
    self.convert_dict_event = convert_dict_event # True
    self.extend_enc_seq = extend_enc_seq # False
  ...
  def register_segments(self):
    ...
    # print(len(self.piece_segments), self.piece_segments) # each song choose (start, end) for training

  def get_sample_from_file(self, piece_idx):
    piece_evs = pickle_load(self.pieces[piece_idx])[1]
    print("price_evs:", len(piece_evs))
...
def main():
  ...
  logging.basicConfig(
      stream=sys.stdout,
      level=logging.ERROR if False else logging.INFO,
      format="%(message)s",
  )
  ...
  dataset = SkylineFullSongTransformerDataset(
